Sub update_EKs()
    ...
    For iRows = rowStart To NumRows
        ...
        Else ' = search result found
            ...
            ' ### check if new price is a lot higher than old price
            ' wie viel teurer newPrice ist
            ' zwischen maximal 4,36 % Preiserhöhung übernehmen
            ' darüber eigenen Log Code geben
            ...
            If proz <= maxProz Then
                ...
                If Cells(iRow, columnChanged) = "1" Then
                    Cells(iRow, columnChanged) = "4"
                ElseIf Cells(iRow, columnChanged) = "2" Then
                    Cells(iRow, columnChanged) = "5"
                ElseIf Cells(iRow, columnChanged) = "3" Then
                    Cells(iRow, columnChanged) = "6"
                Else
                    Cells(iRow, columnChanged) = "7"
                End If
